...
from .utils import namehash, ping
...
@ping
@app.route('/user/', methods=['GET', 'POST'], defaults={'username': None})
...
def jobs():
    ...
    for key, schema in schemas.items():
        ...
        jobs = JobTable(jobs, target='jobs', exlude=['key', 'delete'])
        all_jobs[key] = jobs
...
def summary():
    ...
    for c in content:
        c['correlation'] = url_for('summary_image', which='correlation', **c)
        c['average'] = url_for('summary_image', which='average', **c)
        if reso.Activity() & c:
            c['trace'] = url_for('traces',
...
@ping
@app.route('/image/<animal_id>/<session>/<scan_idx>/<slice>/<reso_version>/<which>')
